...
fluid = 1; solid = 2; inlet = 3; outlet = 4; sides = 5; clamp = 6;

# Load the mesh file (the mesh can be curved):
mymesh = mesh("fsimicropillar.msh")

# Define the fluid-structure interface:
fsinterface = selectintersection([fluid, solid])
...
v = field("h1xy"); p = field("h1"); u = field("h1xy"); umesh = field("h1xy"); y = field("y");
...
fsi += integral(fluid, umesh, predefinednavierstokes(dof(v), tf(v), v, dof(p), tf(p), mu, rhof, 0, 0, 0) )
...
uprev = 0.0; umax = 1.0;
while (abs(umax-uprev)/abs(umax)).evaluate() > 1e-6:
    ...
    print("Max pillar deflection (relative change):", umax*1e6, "um (", (abs(umax-uprev)/abs(umax)).evaluate(), ")")
...
print((umax < 8.384e-06) and (umax > 8.383e-06))
