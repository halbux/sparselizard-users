...
conductor1 = 1; conductor2 = 2; insulator = 3; steel = 4; air = 5; bnd = 6;
...
notmagnetic = selectunion([air, conductor1, conductor2, insulator])
...
hbcurve = spline(bdata, hdata)

# Define nu as h = nu * b and fix the 0/0 division for the first entry:
nudata = np.zeros(len(hdata))
for i in range(1, len(hdata)):
    nudata[i] = hdata[i]/bdata[i]
nudata[0] = nudata[1]
# Use a cubic spline interpolation for nu(b):
nucurve = spline(bdata, nudata)

# Define nu on each region:
nu = parameter()

nu.setvalue(notmagnetic, 1/(4*getpi()*1e-7))
nu.setvalue(steel, expression(nucurve, norm(b))) # norm(b) is the abscissa

h = nu * b
dhdb = expression(hbcurve.getderivative(), norm(b))
...
print("Total current in each conductor is", js*expression(1).integrate(conductor1, 5), "A")

# Initial solution is a = 0 (thus b = 0):
x = vec(magnetostatics)

# Nonlinear iteration:
relres = 1.0; maxb = 0.0; iter = 0;
while relres > 1e-5:
...
print(abs(maxb - 1.75962).evaluate()/1.75962 < 1e-5)
