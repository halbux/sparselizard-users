...
pztlayer = 1; siliconlayer = 2; electrode = 3; ground = 4; clamp = 6; freeside = 7; elecbox = 8; mecabox = 9;

mymesh = mesh("bilayer.msh")

wholedomain = selectunion([pztlayer, siliconlayer])
...
rhosi = 2330.0; rhopzt = 7500.0;

# Diagonal relative permittivity matrix for PZT:
K = expression(3,3,[1704,1704,1433])
K = K * 8.854e-12
...
Hsi = expression(6,6, [194.5e9, 35.7e9,194.5e9, 64.1e9,64.1e9,165.7e9, 0,0,0,79.6e9, 0,0,0,0,79.6e9, 0,0,0,0,0,50.9e9])
Hpzt = expression(6,6, [1.27e11, 8.02e10,1.27e11, 8.46e10,8.46e10,1.17e11, 0,0,0,2.29e10, 0,0,0,0,2.29e10, 0,0,0,0,0,2.34e10])
...
umax = norm(u.harmonic(2)).max(wholedomain, 5)[0]
print("Peak deflection is", umax, "m")
...
print((umax < 1.7386e-07) and (umax > 1.7384e-07))
